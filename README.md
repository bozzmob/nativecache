...
- Native in-application cache: keep cache reads/writes inside your app process.
- No external Redis process required for common local/test/app-cache use cases.
- Lower operational overhead for simple caching needs (no separate service to provision, monitor, or connect to).
- Fast in-memory access with Redis-style command semantics.

## When Should You Use NativeCache?

Use NativeCache when:

- You need an in-app cache for API responses, computed data, sessions, or short-lived state.
- You want to avoid network hops and keep cache operations in the same process context.
- You want Redis-like APIs in local development or tests without running infrastructure.
- You need optional local snapshot persistence (`.nativecache/snapshot.json`) for restart continuity.

Consider external Redis instead when:

- You need shared cache state across multiple app instances/processes.
- You need cross-service cache coordination or distributed pub/sub patterns.
- You need Redis server features beyond this in-process library.
...
- [NativeCache](#nativecache)
  - [Benefits](#benefits)
  - [When Should You Use NativeCache?](#when-should-you-use-nativecache)
